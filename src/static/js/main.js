	...
	const app = {
		...
		cached() {
			...
			this.alerts = {
				...
				noUsers2: document.querySelector('[data-label="alert-noUsers2"]'),
				noLogs: document.querySelector('[data-label="alert-noLogs"]'),
				noItems: document.querySelector('[data-label="alert-noItems"]')
			}
		...
		initiateActions() {
			...
			if (this.db.users().get().length <= 0) {
				...
				this.alerts.noUsers2.classList.remove('d-none')
				this.alerts.noUsers2.classList.add('d-flex')
			} else {
				this.alerts.noUsers.classList.remove('d-flex')
				this.alerts.noUsers.classList.add('d-none')
				this.alerts.noUsers2.classList.remove('d-flex')
				this.alerts.noUsers2.classList.add('d-none')
			};

			if (this.db.posLog().get().length <= 0) {
				this.alerts.noLogs.classList.remove('d-none')
				this.alerts.noLogs.classList.add('d-flex')
			} else {
				this.alerts.noLogs.classList.remove('d-flex')
				this.alerts.noLogs.classList.add('d-none')
			};

			if (this.db.items().get().length <= 0) {
				this.alerts.noItems.classList.remove('d-none')
				this.alerts.noItems.classList.add('d-flex')
			} else {
				this.alerts.noItems.classList.remove('d-flex')
				this.alerts.noItems.classList.add('d-none')
			};

			this.userPane.addBtn.addEventListener('click', () => {
				this.addUser()
			});
			this.userPane.rmBtn.addEventListener('click', () => {
				this.removeUser()
			});
			this.userPane.topup.addBtn.addEventListener('click', () => {
				this.topUp()
			});

			this.itemPane.addBtn.addEventListener('click', () => {
				this.addItem()
			});
			this.itemPane.rmBtn.addEventListener('click', () => {
				this.removeItem()
			});
			...
			this.pos.btnConfirm.addEventListener('click', () => {
				this.posPay(this.userPane.user.selected, this.pos.calculate.newCreditStatus, this.pos.calculate.toPay)
			});
			this.pos.btnCancel.addEventListener('click', () => {
				this.readyState()
			});

			this.setPane.removeData.btn.addEventListener('click', () => {
				this.removeData()
			});
			...
			this.setPane.backup.import.addEventListener('click', () => {
				this.importBackup()
			});

			this.setPane.sudo.goSudo.addEventListener('click', () => {
				this.goSudo()
			});
		...
		generateUsers() {
			...
			if (this.db.users().get().length != 0) {
				this.db.users().order("date desc").each((r, index) => {
					...
					if (r.credit <= 0) {
						tr.classList.add('user-credit-neg')
					};
					tr.innerHTML = `
                        <td>${r.name}</td>
		...
		removeUser() {
			...
			this.db.users({
				___id: this.userPane.user.selected
			}).remove();
			this.readyState();
		},
		...
		topUp() {
			...
			this.db.users(this.userPane.user.selected).update({
				credit: parseFloat(creditCurrent) + this.userPane.topup.amount
			});
			this.readyState();
		},
		...
		removeItem() {
			...
			this.db.items({
				___id: this.itemPane.item.selected
			}).remove();
			this.readyState();
		},
		...
		posPay(selectedUser, newCredit, pay) {
			this.setTime();
			this.db.users(selectedUser).update({
    ...
    "users": [
		...
		importBackup() {
			function loadFile() {
				...
				if (!input) {
					createToast('Backup', 'Fout bij het importeren')
				} else if (!input.files) {
					createToast('Backup', 'Deze browser ondersteund deze functie niet')
				} else if (!input.files[0]) {
					createToast('Backup', 'Upload eerst een bestand')
				} else {
					file = input.files[0];
					fr = new FileReader();
